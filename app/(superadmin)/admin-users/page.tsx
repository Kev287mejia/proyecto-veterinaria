...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('id, full_name, phone, role, created_at, vet_clinics(id, clinic_name)')
  ...
  const handleClinicChange = async (userId: string, newClinicId: string) => {
    try {
      ...
      await fetchUsers(); // Refresh to reflect new associations
      alert("Vinculación de clínica actualizada.");
    } catch (error: any) {
      console.error("Error linking user to clinic:", error);
      alert("Error al vincular la clínica: " + error.message);
    }
  };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low border-b border-outline-variant/60">
                ...
                <th className="p-4 font-bold text-on-surface text-sm uppercase tracking-wider">Clínica Vinculada</th>
                <th className="p-4 font-bold text-on-surface text-sm uppercase tracking-wider">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/30">
              {filteredUsers.map((u) => {
                // Determine linked clinic
                const assignedClinic = u.vet_clinics && u.vet_clinics.length > 0 ? u.vet_clinics[0] : null;
                ...
                return (
                  <tr key={u.id} className="hover:bg-surface-container-low/30 transition-colors">
                    ...
                    <td className="p-4">
                      {u.role === 'vet' || u.role === 'admin' ? (
                        <select
                          value={assignedClinic?.id || ''}
                          onChange={(e) => handleClinicChange(u.id, e.target.value)}
                          className="bg-surface-container border border-outline-variant text-on-surface text-sm rounded-xl px-3 py-2 focus:ring-2 focus:ring-primary focus:outline-none max-w-[200px]"
                        >
                        ...
                        <span className="text-xs text-on-surface-variant italic font-semibold">N/A (Dueño)</span>
                      )}
                    </td>
                    <td className="p-4">
