...
export default function VetMascotas({ userId }: { userId: string }) {
  ...
  const getStatusColor = (status: Pet['status']) => {
    switch (status) {
      case 'Saludable': return 'bg-secondary shadow-[0_0_8px_rgba(0,108,73,0.4)]';
      case 'En Observación':
      case 'Crítico': return 'bg-error shadow-[0_0_8px_rgba(186,26,26,0.4)]';
      case 'En Tratamiento': return 'bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.4)]';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
        <div>
          <h2 className="font-headline-lg text-[28px] md:text-[32px] text-primary tracking-tight font-bold">Directorio de Pacientes</h2>
      ...
      <section className="flex flex-col md:flex-row flex-wrap items-center gap-4 p-4 bg-white/70 backdrop-blur-md border border-outline-variant/40 rounded-2xl">
        ...
        <div className="ml-auto text-on-surface-variant text-sm font-medium">
          <span className="font-bold text-primary">{filteredPets.length}</span> Pacientes
        </div>
      </section>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          >
            ...
            <h3 className="font-semibold text-lg text-on-surface-variant group-hover:text-primary">Registrar Nuevo</h3>
            <p className="text-on-surface-variant text-sm mt-1">Ingresa un paciente al sistema</p>
          </div>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-primary/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-card w-full max-w-lg p-8 rounded-2xl shadow-xl border border-outline-variant/60 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-2xl text-primary">Registrar Nuevo Paciente</h3>
              <button
                onClick={() => {
                  setIsAddModalOpen(false);
                  setAvatarFile(null);
            ...
            <form onSubmit={handleAddPet} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-on-surface mb-1.5" htmlFor="photo">Foto del Paciente (Opcional)</label>
                <input
                  id="photo"
                  type="file"
                  ...
                  className="w-full px-4 py-2.5 rounded-lg border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary focus:border-primary transition-all outline-none text-sm text-on-surface cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"
                />
              </div>
